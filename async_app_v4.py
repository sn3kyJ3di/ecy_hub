...
# Basic authentication credentials for ECY device
device_username = "nv_vis"
device_password = "p55FBTws+cvujL:"

# List of ECY Device IP Addresses
device_ip_addresses = []
device_cookies = {}
cache = {}
...
async def fetch_device_data(session, ip_address):
    ...
    try:
        ...
        async with session.post(url, headers=headers, json=body, ssl=False) as response:
            ...
            # Extract and structure the data from the responses array
            structured_data = {}
...
def remove_entries(d, keys_to_remove, substrings_to_remove=None):
    """
    Recursively remove entries with specified keys and their children from a dictionary.
    Optionally, remove entries that contain any of the specified substrings in their keys.
    
    :param d: The dictionary to process.
    :param keys_to_remove: A set of keys to remove.
    :param substrings_to_remove: A list of substrings to check for in keys to remove (optional).
    :return: The cleaned dictionary.
    """
    if not isinstance(d, dict):
        return d

    # Create a new dictionary to avoid modifying the original one
    new_dict = {}
    ...
    for ip, content in data.items():
        if "Local BACNet Data" in content:
            for device_id, device_data in content["Local BACNet Data"].items():
                ...
                # Add the device to the nodes dictionary with its IP address
                nodes[device_id] = {
                ...
                # Copy data from Remote BACnet Data if it exists
                if "Remote BACnet Data" in content:
                    for remote_id, remote_data in content["Remote BACnet Data"].items():
                        ...
                        # Add count of object-names for Objects
        ...
        # Populate Network Values using Local GFX Network Values
                ...
                for gfx_value in gfx_values:
                    ...
                    if len(parts) > 5 and parts[2] == 'bacnet' and parts[3] == 'remote' and parts[4] == 'devices':
                        ...
                        if object_id not in links["Network Values"][local_device_id][remote_device_id]["objects"][object_type]:
                            # Cross-reference with Objects to get the object-name
                            object_name = ""
                            if local_device_id in links["Objects"] and remote_device_id in links["Objects"][local_device_id] and "objects" in links["Objects"][local_device_id][remote_device_id] and object_type in links["Objects"][local_device_id][remote_device_id]["objects"] and object_id in links["Objects"][local_device_id][remote_device_id]["objects"][object_type]:
                                ...
                                object_name = "N/A"  # Provide a default value when object name is not found
                        ...
                        # Check if the remote_device_id exists in nodes and has a "connection-status" property
        ...
        # Populate Modbus Nodes and Objects using Remote Modbus Data
        if "Remote Modbus Data" in content:
            for modbus_key, modbus_value in content["Remote Modbus Data"].items():
                if modbus_value != "error" and modbus_value:
                    ...
                    # Rename "key" to "object-name" in points
                    ...
                    # Copy the entire object from Remote Modbus Data into Links/Objects/
        ...
        # Populate Modbus Network Values using Local GFX Modbus Values
            ...
            for gfx_key, gfx_values in content["Local GFX Modbus Values"].items():
                ...
                for gfx_value in gfx_values:
                    ...
                    if len(parts) > 3 and parts[2] == 'modbus' and parts[3] == 'devices':
                        ...
                        # Increment the count by 1 for each unique device
        ...
        if "Weather Connections" in content:
            ...
            for weather_key, weather_value in weather_data.items():
                if weather_value != "error" and weather_value:
                    ...
                    # Capitalize the first letter of each word in the provider name
        ...
        if "Internet & NTP Connection" in content:
            ...
            # Create a unique Internet node name for each device
            local_device_id = list(content["Local BACNet Data"].keys())[0]
            ...
            # Assign Internet data only to the corresponding device
            ...
            # Assign NTP data only to the corresponding device
            if local_device_id in links["Objects"]:
        ...
        if "BBMD Table Connections" in content:
            ...
            # Process primary BBMD connections
            ...
            for bbmd_key, bbmd_value in primary_bbmd_data.items():
                ...
                # Check if the BBMD's IP address matches any IP address in the data
                ...
                if local_device_id and bbmd_device_id:
                    # BBMD node has the same IP address as a local node, add to existing links
                    if local_device_id not in links["Objects"]:
                        links["Objects"][local_device_id] = {}
                    ...
                    # BBMD node is a new node, create a new node and add to links
            ...
            # Process secondary BBMD connections
...
def count_object_names(obj):
    """
    Count the number of object-name entries in a nested dictionary.
    
    :param obj: The dictionary to count object-names in.
    :return: The count of object-name entries.
    """
    count = 0
...
def reorganize_data(data):
    for ip, content in data.items():
        if "Local BACNet Data" in content:
            # Move the descendants of Local BACNet Data under Nodes
            for local_id, local_data in content.pop("Local BACNet Data").items():
                ...
                if local_id in data["Nodes"]:
                    # Merge node-type and ip-address if it already exists
        ...
        if "Remote BACnet Data" in content:
            # Move the descendants of Remote BACnet Data under Nodes
...
async def fetch_all_data(device_ip_addresses):
    print(f"Fetching data for IP addresses: {device_ip_addresses}")  # Add this line
    async with aiohttp.ClientSession() as session:
        ...
        # Remove entries with keys 4194303 and 4194304 and their children
        keys_to_remove = {"4194303", "4194304"}  # Use string keys for comparison
        ...
        # Reorganize the cleaned data
        reorganized_data = reorganize_data(data_with_links)
        
        # Remove Local GFX Network Values, Remote BACnet Data, and Remote Modbus Data sections
        ...
        # Remove entries with specified substrings from the "Nodes" section
        ...
        # Remove leftover IP addresses and their descendants
...
async def get_data():
    ...
    if not initial_fetch_completed:
        # Fetch all data if the initial fetch is not completed
...
if __name__ == '__main__':
    # Pre-fetch all data to cache on startup
    loop = asyncio.get_event_loop()
    all_network_values_global = loop.run_until_complete(fetch_all_data(device_ip_addresses))
    initial_fetch_completed = True

    # Start the periodic fetch task in a background thread
